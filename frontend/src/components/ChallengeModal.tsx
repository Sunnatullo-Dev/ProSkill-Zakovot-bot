...
import { challengeTeamByCode } from "../api/client";
...
export default function ChallengeModal({ onClose, onChallenged }: ChallengeModalProps) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    setError("");
    const result = await challengeTeamByCode(trimmed);
    setSubmitting(false);
    ...
    >
      ...
      >
        <div style={{ fontSize: "18px", fontWeight: 800, color: "var(--text)", marginBottom: "4px" }}>
          ⚔️ Bellashuvga taklif
        </div>
        <div style={{ fontSize: "13px", color: "var(--muted)", marginBottom: "14px" }}>
          Raqib jamoaning 6 belgili kodini kiriting:
        </div>
        <input
          autoFocus
          maxLength={6}
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              background:
                code.trim().length < 4 || submitting ? "var(--border)" : "var(--accent)",
              color: "white",
              fontSize: "14px",
              fontWeight: 800,
